...
export const playlistsRouters = createTRPCRouter({
  ...
  addVideo: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if(existingPlaylistVideo){
        throw new TRPCError({ code: "CONFLICT", message: "Video already added to playlist" });
      }
                ...
                containsVideo: videoId
                  ? sql<boolean>`(
                    SELECT EXISTS(
                      ...
                      WHERE
                        pv.playlist_id = ${playlists.id} AND
                        pv.video_id = ${videoId}
                    )
                  )`
                  : sql<boolean>`false`,
